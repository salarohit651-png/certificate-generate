...
export async function PUT(
  ...
  try {
    ...
    const updateData: any = {
      ...
      passoutPercentage: Number.parseFloat(
        formData.get('passoutPercentage') as string,
      ),
...
export async function DELETE(
  ...
  try {
    ...
    console.log(
      ...
      userExists
        ? `Found user: ${userExists.name} (${userExists.emailId})`
        : 'User not found',
    );
    ...
    if (!deletionResult) {
      console.error(
        `[v0] Deletion failed - no user was deleted for ID: ${params.id}`,
      );
      return NextResponse.json(
    ...
    if (verifyDeletion) {
      return NextResponse.json(
        { success: false, message: 'User deletion verification failed' },
        { status: 500 },
      );
    }
